		...
		while(true){
			...
			case 20:// it may be a TAG_MACRO or a  EXP_MACRO
				...
				if(c.equals("i")){
					...
					errors.add(new LexerError(lookahead, "'i' ou 'e'", c, LexerErrorType.WrongCharacter));
					rollback();
					return null;
				}
				break;
			case 21: 
				...
				if (c.equals("f")) {
					...
					//  error: esperava um f
					errors.add(new LexerError(lookahead, "f", c, LexerErrorType.WrongCharacter));
					rollback();
					return null;
				}
				break;
			case 22:
				...
				if(compare(c,Symbols.WhiteSpace)){ // #if
					...
					errors.add(new LexerError(lookahead, "' ', 'n', or 'd'", c, LexerErrorType.WrongCharacter));
					rollback();
					return null;
				}
				break;
...
//				rollback();
			...
			case 24:
				...
				if (c.equals("e")) { // #ifde or #ifnde or #elifnde or #elifde
					...
					// error: esperava e
					errors.add(new LexerError(lookahead, "'e'", c, LexerErrorType.WrongCharacter));
					rollback();
					return null;
				}
				break;
			case 25: 
				...
				if(c.equals("d")){ // #ifnd
					...
					// error: esperava n
					errors.add(new LexerError(lookahead, "'n'", c, LexerErrorType.WrongCharacter));
					rollback();
					return null;
				}
				break;
			case 26:
				c = nextChar();
				if (c.equals("f")) { // #ifdef or #ifndef or #elifndef or #elifdef
					buffer += c;
					status = 27;
				}else {
					// error: esperava f
...
//				rollback();
			...
			case 28:
				...
				if(c.equals("l")){ // #el
					...
					// error: esperava l
					errors.add(new LexerError(lookahead, "'l'", c, LexerErrorType.WrongCharacter));
					rollback();
					return null;
				}
				break;
			case 29: 
				...
				if(c.equals("i")){ // #eli
					...
					// error: esperava i
					errors.add(new LexerError(lookahead, "'i'", c, LexerErrorType.WrongCharacter));
					rollback();
					return null;
				}
				break;
...
//				rollback();
			...
			case 48:
				...
				if (compare(c, Symbols.Letter) 
					...
					errors.add(new LexerError(lookahead, "<Letter> or '('", c, LexerErrorType.WrongCharacter));
					rollback();
					return null;
				}
			case 49:
			...
			case 99: // Error: Unknown character
				errors.add(new LexerError(lookahead, "<Letter>, '/', '#', '&' or '|'", c, LexerErrorType.InvalidCharacter));
				rollback();
				buffer = "";
				status = 0;
				return null;
	...
	private static TokenType chooseTokenType(String buffer) {
		...
		else if(buffer.equals("#ifdef") || 
				buffer.equals("#ifndef") || 
				buffer.equals("#elifdef") || 
				buffer.equals("#elifndef")
				)
			return TokenType.TAG_MACRO;
		else
			// TODO error: não devia ter outro
		errors.add(new LexerError(lexemeStartPoint, "'#if', '#elif', '#ifdef', "
				+ "'#ifndef', '#elifdef' or '#elifndef' ", c, LexerErrorType.InvalidMacro));
		return null;
	}
